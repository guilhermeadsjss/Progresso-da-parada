...
# --- Logo (procura por arquivos comuns de logo na mesma pasta) ---
...
def find_col(df, keywords):
    """Retorna o primeiro nome de coluna que contenha alguma das palavras-chave (já normalizadas)."""
    for c in df.columns:
        for k in keywords:
            if k in c:
                return c
...
def carregar_dados():
    ...
    try:
        ...
        # Tentar ler a primeira sheet como fallback
    ...
    # Detectar quais colunas usar (palavras-chave)
    col_area = find_col(df, ['area', 'sistema'])
    col_desc = find_col(df, ['descricao', 'desc', 'descricao_', 'descricao_atividade', 'descricao_atividade', 'atividade', 'description'])
    col_status = find_col(df, ['status'])
    col_tipo = find_col(df, ['tipo', 'medicao', 'medicao'])
    col_m2_prev = find_col(df, ['m2_previsto', 'm2_prev', 'previsto', 'm2'])
    col_m2_real = find_col(df, ['m2_realizado', 'm2_real', 'realizado'])
    col_pct = find_col(df, ['conclusao', 'percent', 'porcent', 'porcentagem', 'pct', 'percentual'])

    # Criar colunas padrão se não existirem (evita KeyError)
    if col_area is None:
    ...
    # col_tipo pode ficar None — tratamos depois
...
if df.empty:
    ...
    # mostrar colunas detectadas (ajuda no debug)
    with st.expander("🔎 Colunas detectadas e mapeamento (útil para debug)"):
        st.write("Colunas normalizadas detectadas:", list(df.columns))
        st.write("Mapeamento de colunas usadas:")
        st.write({
    ...
    # Definir o conjunto de linhas consideradas "m2" (se coluna de tipo existir, filtra; caso contrário considera todas)
    if col_tipo:
        df_m2 = df_filtrado[df_filtrado[col_tipo].astype(str).str.contains('m', case=False, na=False)]
    else:
        df_m2 = df_filtrado.copy()

    m2_previsto = df_m2[col_m2_prev].sum() if col_m2_prev in df_m2.columns else 0
    m2_realizado = df_m2[col_m2_real].sum() if col_m2_real in df_m2.columns else 0
    ...
    with col_chart1:
        ...
        if col_area and col_m2_real in df.columns:
            progresso_area = df_m2.groupby(col_area)[col_m2_real].sum()
            if not progresso_area.empty:
                st.bar_chart(progresso_area)
            else:
                st.write("Não há dados de m² para exibir por área.")
        else:
            st.write("Coluna de área ou m² realizado não encontrada para gráfico.")
    ...
    for _, row in df_filtrado.iterrows():
        ...
        with st.expander(f"🔹 {area_val} - {desc_val}"):
            ...
            # Mostrar linha completa em tabela pequena se quiser detalhar mais
